...
void loadcb(int row, int col, char *lab, char *invlab, int type)
{
	...
	cb->invlabel = invlab;
	cb->type = type;
	cb->b = muiNewButton(Bhspace + col * (Bhspace + Bwidth), (col + 1) * (Bhspace + Bwidth), Bvspace + row * (Bvspace + Bheight), (row + 1) * (Bvspace + Bheight));
...
void initkeypad(void)
{
	...
	loadcb(7, 4, "e", "e", Ekey);
	loadcb(7, 5, "f", "f", Fkey);
	tl = muiNewTextList(Bhspace, HEIGHT * Bheight + (HEIGHT + 1) * Bvspace, WIDTH * (Bhspace + Bwidth), Displaylines);
...
void initcalc(void)
{
	...
	glutInitWindowSize(WIDTH * Bwidth + (WIDTH + 1) * Bhspace, HEIGHT * Bheight + (HEIGHT + 2) * Bvspace + Displaylines * 20 + 7);
...
{
	if (calcdata[s] - trunc(calcdata[s]) == 0 && (calcdata[s] < 1.0e9 && -1.0e9 < calcdata[s])) {
		if (base == 10) {
		...
		if (calcdata[s] < 1.0e9 && -1.0e9 < calcdata[s] && (calcdata[s] > 1.0e-5 || calcdata[s] < -1.0e-5))
...
void kbd(unsigned char c, int x, int y)
{
	...
	case '6':
		...
		break;
	case '7':
		interpclick(Sevenkey);
		break;
	case '8':
		interpclick(Eightkey);
		break;
	case '9':
		interpclick(Ninekey);
		break;
	...
	case '+':
		...
		break;
	case '-':
		interpclick(Minuskey);
		break;
	case '*':
		interpclick(Timeskey);
		break;
	case '/':
		interpclick(Dividekey);
		break;
	case '\n':
	case '\r':
		interpclick(Enterkey);
		break;
	}
...
void binop(char c)
{
	...
	case '^':
		...
		break;
	case '&':
		if (calcdata[CSP - 1] - trunc(calcdata[CSP - 1]) != 0)
			return;
		if (calcdata[CSP] - trunc(calcdata[CSP]) != 0)
			return;
		i = calcdata[CSP - 1];
		j = calcdata[CSP];
...
void unop(int c)
{
	...
	case Sinkey:
		if (invmode) {
			...
			if (degreemode)
				calcdata[CSP] *= 180.0 / M_PI;
		}
		else {
			if (degreemode)
				calcdata[CSP] *= M_PI / 180.0;
...
void interpclick(int x)
{
	...
	if (savingprog && x != Progkey && x != Runkey)
		program[proglen++] = x;
	if ((Zerokey <= x && x <= Ninekey) || x == Dotkey || (base == 16) && (Akey <= x && x <= Fkey)) {
		if (enablepush)
			push();
		enablepush = 0;
		if (x == Dotkey)
