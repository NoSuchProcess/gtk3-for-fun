...
static gboolean scribble_motion_notify_event(GtkWidget *widget, GdkEventMotion * event, gpointer data)
{
	...
	/* This call is very important; it requests the next motion event. If you don't call gdk_window_get_pointer() you'll only get a single motion event. The reason is that we specified
	   GDK_POINTER_MOTION_HINT_MASK to gtk_widget_set_events(). If we hadn't specified that, we could just use event->x, event->y as the pointer location. But we'd also get deluged in events. By
	   requesting the next event as we handle the current one, we avoid getting a huge number of events faster than we can cope. */
...
static gboolean checkerboard_motion_notify_event(GtkWidget *widget, GdkEventMotion * event, gpointer data)
{
	int x, y;
	GdkModifierType state;
...
static gboolean checkerboard_button_press_event(GtkWidget *widget, GdkEventButton * event, gpointer data)
{
...
static gboolean checkerboard_draw(GtkWidget *da, cairo_t *cr, gpointer data)
{
...
#define SPACING 2
	...
	/* At the start of a draw handler, a clip region has been set on the Cairo context, and the contents have been cleared to the widget's background color. The docs for
	...
	while (i < width) {
		...
		while (j < height) {
			...
			else if (ycount != 10 || xcount != 10)
				cairo_set_source_rgb(cr, 1, 1, 1);
			else
				cairo_set_source_rgb(cr, 0, 0.45777, 0.45777);

			/* If we're outside the clip, this will do nothing. */
			cairo_rectangle(cr, i + offset_x, j + offset_y, CHECK_SIZE, CHECK_SIZE);
			cairo_fill(cr);
...
GtkWidget *do_drawingarea(GtkApplication *app, gpointer user_data)
{
	GtkWidget *frame;
	GtkWidget *vbox;
	GtkWidget *da;
...
int main(int argc, char *argv[])
{
	...
	g_signal_connect(G_OBJECT(app), "activate", G_CALLBACK(do_drawingarea), NULL);
